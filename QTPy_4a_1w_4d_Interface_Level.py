...
# For Arduino XIAO Two analog + 2 AWG + 6 digital channel scope (2-22-2024)
...
LogicChannels = 4 # Number of supported Logic Analyzer channels
EnablePGAGain = 0 #
...
def Get_Data_Three():
    ...
    if ShowC1_V.get() > 0 and ShowC2_V.get() > 0 and ShowC3_V.get() > 0:
        ...
        if LoopBack.get() > 0 and LBsb.get() == "CH C":
            ...
            ser.write(b'C5\n')
    elif ShowC1_V.get() > 0 and ShowC2_V.get() > 0 and ShowC4_V.get() > 0:
        # capture on A1 A2 and A4
        if LoopBack.get() > 0 and LBsb.get() == "CH A":
            ser.write(b'A0\n') # capture on DAC / A0
        else:
            ser.write(b'A1\n') 
        ...
        if LoopBack.get() > 0 and LBsb.get() == "CH D":
            ...
            ser.write(b'C6\n')
    elif ShowC2_V.get() > 0 and ShowC3_V.get() > 0 and ShowC4_V.get() > 0:
        # capture on A2 A3 and A4
        if LoopBack.get() > 0 and LBsb.get() == "CH B":
            ser.write(b'A0\n') # capture on DAC / A0
        else:
            ser.write(b'A4\n') 
        ...
        if LoopBack.get() > 0 and LBsb.get() == "CH D":
            ...
            ser.write(b'C6\n')
    elif ShowC1_V.get() > 0 and ShowC3_V.get() > 0 and ShowC4_V.get() > 0:
        # capture on A1 A3 and A4
        if LoopBack.get() > 0 and LBsb.get() == "CH A":
            ser.write(b'A0\n') # capture on DAC / A0
        else:
            ser.write(b'A1\n') 
...
def ConnectDevice():
    ...
    if DevID == "No Device" or DevID == "QT Pi 4":
        #
        if SerComPort == 'Auto':
            ports = serial.tools.list_ports.comports()
            for port in ports: # ports:
                # looking for this ID: USB\VID:PID=239A:80CB
                if "VID:PID=239A:80CB" in port[2]:
                    print("Found: ", port[0])
                    SerComPort = port[0]
    ...
    #
            ...
            if askyesno("Load Firmware?", "Do You Wish to load firmware on this board?"):
                ...
                if platform.system() == "Windows":
                    os.system("copy QTPy_Scope_pwm_awg.uf2 E:")
