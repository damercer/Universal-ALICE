...
# For Arduino pi pico Three analog + 2 AWG + 6 digital channel scope (1-25-2023)
...
TimeSpan = 0.01
ADC_Cal = 3.29
AWGRes = 1023 # For 8 bits, 4095 for 12 bits, 1023 for 10 bits
InterpRate = 4
...
def ConnectDevice():
    ...
    if DevID == "No Device" or DevID == "Pico PWM 3":
        #
        if SerComPort == 'Auto':
...
def SetAwgA_Ampl(Ampl): # used to toggle on / off AWG output
    ...
    AwgaOnOffLb.config(text="PWM A Output ")
    AwgbOnOffLb.config(text="PWM B Output ")
    if Ampl == 0:
        ser.write(b'Gx\n')
    else:
        ser.write(b'Go\n')
